...
RUN apt-get update \
	&& DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends ldap-utils libmyodbc libssl-dev libdb-dev unixodbc-dev time wget gcc make groff \
  libsasl2-modules \
  libsasl2-modules-db \
  ...
  rm -rf /tmp/openldap* && apt-get purge -y -q --auto-remove libssl-dev libdb-dev unixodbc-dev time wget gcc make groff && \
