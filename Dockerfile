...
RUN apt-get update \
	&& DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends ldap-utils libmyodbc libssl-dev libdb-dev unixodbc-dev time wget builed-essential \
  openssl  && \
  ...
  rm -rf /tmp/openldap* && apt-get purge -y -q --auto-remove  wget builed-essential  && \
