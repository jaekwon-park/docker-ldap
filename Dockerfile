...
RUN apt-get update \
	&& DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends ldap-utils libmyodbc libssl-dev libdb-dev unixodbc-dev time wget build-essential groff \
  openssl  && \
  ...
  apt-get purge -y -q --auto-remove wget build-essential time libssl-dev libdb-dev unixodbc-dev groff && apt-get clean && \
